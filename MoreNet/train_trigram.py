...
if not os.path.exists('trigram_cider_model'):
    os.mkdir('trigram_cider_model')

if opts.use_att:
    save_dir = 'trigram_cider_model/' + 'att_model'
else:
    save_dir = 'trigram_cider_model/' + 'fc_model'
...
image_dim = 2048
vocab_size = loader.vocab_size + 2
cell_size = 512
lr = 0.00005
if opts.use_att:
    model = AttModel(batch_size=batch_size, image_dim=image_dim, vocab_size=vocab_size, cell_size=cell_size, lr=lr, ngram=3,on_gpu=True)
    model.load('warm_model/att_warm/model.init')
else:
    model = FCModel(batch_size=batch_size, image_dim=image_dim, vocab_size=vocab_size, cell_size=cell_size, lr=lr, ngram=3,on_gpu=True)
...
while iter < finish_iter:
    ...
    for _ in xrange(20):
        ...
        sample_caps, sample_results = model.trigram_inference(vocab, image_id, feature,
                                                      manner='sample',
                                                      max_length=16)
