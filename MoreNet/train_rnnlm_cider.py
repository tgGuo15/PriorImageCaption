...
while iters < finish_iter:
    ...
    for _ in xrange(30):
        ...
        if opts.use_att:
           sample_caps, sample_results = att_lm_caption(lm,model,image_id,vocab,loader,feature,max_step,'sample')
        else:
            sample_caps, sample_results = lm_caption(lm, model, image_id, vocab, loader, feature, max_step,'sample')
    ...
    mask = np.ones((sample_caps.shape[0]-1,sample_caps.shape[1]))
