...
class AttModel(nn.Module):
    ...
    def initial_state(self, image):
        ...
        first_word = patches.mean(1).squeeze(1)
        ...
        if self.on_gpu:
            ...
            c = c.cuda()
        zero_state = h,c
        return patches, first_word, zero_state
    ...
    def train_on_batch(self, image, sentence, mask, reward):
        ...
        patches, first_word, state = self.initial_state(image)
    ...
    def single_step(self, time_step, state, words, patches, manner='greedy'):
        if time_step != 0:
    ...
    def inference(self, vocab, image_ids, image, manner='greedy',
        ...
        for i in xrange(max_length):
            state, word = self.single_step(i,state, word, patches, manner=manner)
    ...
    def ngram_single_step(self, time_step, state, words, patches,temp_mask, manner='greedy'):
        if time_step != 0:
            words = Variable(torch.LongTensor(words), volatile=True)
            if self.on_gpu:
                words = words.cuda()
    ...
    def fourgram_inference(self, vocab, image_ids, image, manner='greedy',
        ...
        for jj in xrange(max_length):
            ...
            state, word = self.ngram_single_step(jj,state, word, patches,tempmask, manner=manner)
            sentences.append(word)
