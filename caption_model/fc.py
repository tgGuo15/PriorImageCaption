...
class FCModel(nn.Module):
    def __init__(self, batch_size, cell_size, image_dim,
        ...
        if ngram == 3:
            trigram = pickle.load(open('data/features/trigram.pkl'))
            self.trigram_mask = {}
            for tri in trigram:
            ...
            fourgram = pickle.load(open('data/features/fourgram.pkl'))
            self.fourgram_mask = {}
    ...
    def initial_state(self, image):
        ...
        # Image embedding
        first_word = self.image_embedding(image)
        h = Variable(torch.zeros(self.batch_size, self.cell_size))
        c = Variable(torch.zeros(self.batch_size, self.cell_size))
        if self.on_gpu:
            h = h.cuda()
            c = c.cuda()
        zero_state = h, c

        return first_word, zero_state
    ...
    def train_on_batch(self, image, sentence, mask, reward):
        ...
        first_word, state = self.initial_state(image)
        ...
        # Ground-truth
        targets = sentence.data[:, :].view(T*self.batch_size)
    ...
    def single_step(self, time_step,state, words, manner='greedy'):
        if time_step != 0:
    ...
    def inference(self, vocab, image_ids, image, manner='greedy',max_length=16):
        # Choose batch-size
        self.batch_size = image.shape[0]
        ...
        for i in xrange(max_length):
            state, word = self.single_step(i,state, word, manner=manner)
    ...
    def ngram_single_step(self,time_step,state, words,temp_mask, manner='greedy'):
        if time_step != 0:
            words = Variable(torch.LongTensor(words), volatile=True)
            if self.on_gpu:
                words = words.cuda()
    ...
    def fourgram_inference(self, vocab, image_ids, image, manner='greedy',max_length=16):
        # Choose batch-size
        self.batch_size = image.shape[0]
        ...
        # Iteratively generate words
        init_word, state = self.initial_state(image)
        sentences = []
        word = init_word
        ...
        for jj in xrange(max_length):
            ...
            state, word = self.ngram_single_step(jj,state, word,temp_mask,manner=manner)
